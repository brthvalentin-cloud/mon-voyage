  ...
  return (
    <div className="section">
      ...
      {todos.length === 0 && (
        <div className="empty-state">
          <div className="empty-icon">📋</div>
          <p className="empty-title">Votre liste de taches</p>
          <p className="empty-sub">Passeport, visas, reservations... tout ici !</p>
        </div>
      )}
      <div className="todo-list">
        {todos.filter((t) => !t.done).map((t) => {
          ...
          return (
            <div key={ri} className="todo-item">
              <button className="todo-check" onClick={() => toggle(ri)} />
              <span className="todo-text">{t.text}</span>
              <button className="btn-icon small" onClick={() => remove(ri)}>🗑️</button>
            </div>
          );
        })}
        {todos.filter((t) => t.done).length > 0 && (
          <>
            ...
            {todos.filter((t) => t.done).map((t) => {
              const ri = todos.findIndex((x) => x === t);
              return (
                <div key={ri} className="todo-item done">
                  <button className="todo-check checked" onClick={() => toggle(ri)}>✓</button>
                  <span className="todo-text">{t.text}</span>
                  <button className="btn-icon small" onClick={() => remove(ri)}>🗑️</button>
                </div>
              );
...
function VoyageList({ voyages, onCreate, onSelect, onDelete }) {
  ...
  const list = Object.entries(voyages).sort(
      ...
      <div className="section">
        {!showForm && (
          <button className="btn-new-voyage" onClick={() => setShowForm(true)}>
            <span className="btn-new-icon">+</span> Nouveau voyage
          </button>
        )}
        {showForm && (
          <div className="form-card">
