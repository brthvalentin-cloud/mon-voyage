import { useState, useEffect, useRef } from “react”; import { initializeApp } from “firebase/app”; import { getDatabase, ref, onValue, set, remove as fbRemove } from “firebase/database”;
...
const TABS = [“Planning”, “Budget”, “To-Do”]; const TAB_ICONS = [“📅”, “💰”, “✅”]; const defaultVoyageData = { stops: [], budget: { items: [] }, todos: [] };
...
{ id: “restaurant”,label: “Restaurant”, emoji: “🍽️”, color: “#E67E22” }, ]; const getCat = (id) => STOP_CATEGORIES.find(c => c.id === id) || STOP_CATEGORIES[0];

function useVoyages() {
const [voyages, setVoyages] = useState({}); const [loaded, setLoaded] = useState(false);
useEffect(() => {
...
const createVoyage = (name, destination, dates) => { const id = “voyage_” + Date.now(); set(ref(db, `voyages/${id}`), { meta: { name, destination, dates, createdAt: Date.now() }, …defaultVoyageData }); return id; }; const deleteVoyage = (id) => fbRemove(ref(db, `voyages/${id}`)); const saveVoyageKey = (id, key, value) => set(ref(db, `voyages/${id}/${key}`), value); return { voyages, loaded, createVoyage, deleteVoyage, saveVoyageKey }; }

// ─── STOP FORM ────────────────────────────────────────────────────────────────
function StopForm({ initial, onSave, onCancel, title }) { const [form, setForm] = useState(initial || { ville: “”, date: “”, note: “”, categorie: “avion” });

return (
<div className="form-card">
<div className="form-header">
<span>{title}</span>
<button className="btn-close" onClick={onCancel}>✕</button> </div> <div className="form-body"> <div className="input-group"> <label className="input-label">Catégorie</label>
<div className="cat-picker">
{STOP_CATEGORIES.map(cat => (
<button
key={cat.id}
className={`cat-btn${form.categorie === cat.id ? " cat-btn-active" : ""}`} style={form.categorie === cat.id ? { borderColor: cat.color, background: cat.color + “18” } : {}}
onClick={() => setForm({ …form, categorie: cat.id })} type=“button”
>
...
<input className=“inp” placeholder=“Ex: Vol Paris→Rome, Musée du Louvre…” value={form.ville} onChange={(e) => setForm({ …form, ville: e.target.value })} /> </div> <div className="input-group"> <label className="input-label">Date</label>
<input className=“inp” type=“date” value={form.date} onChange={(e) => setForm({ …form, date: e.target.value })} /> </div> <div className="input-group"> <label className="input-label">Note</label>
<input className=“inp” placeholder=“Détails, confirmation, adresse…” value={form.note} onChange={(e) => setForm({ …form, note: e.target.value })} /> </div> <button className=“btn-primary” onClick={() => { if (form.ville.trim()) onSave(form); }}>Enregistrer</button> </div> </div> ); }
...
const [editIdx, setEditIdx] = useState(null); const [showAdd, setShowAdd] = useState(false); const [addDate, setAddDate] = useState(””); const [draggingKey, setDraggingKey] = useState(null);

const dragItem = useRef(null);
const dragOver = useRef(null);
// True only when the touch started on the drag handle const touchOnHandle = useRef(false); const timelineRef = useRef(null);

const addStop = (form) => {
const updated = sortStops([…stops, form]); save(“stops”, updated); setShowAdd(false); setAddDate(””); };

const editStop = (form) => {
const updated = sortStops(stops.map((s, i) => (i === editIdx ? form : s))); save(“stops”, updated); setEditIdx(null); };

const remove = (i) => save(“stops”, stops.filter((_, idx) => idx !== i));

const withDate = stops.map((s, i) => ({ …s, originalIdx: i })).filter(s => s.date); const withoutDate = stops.map((s, i) => ({ …s, originalIdx: i })).filter(s => !s.date);

const grouped = withDate.reduce((acc, stop) => { const key = stop.date; if (!acc[key]) acc[key] = []; acc[key].push(stop); return acc; }, {});
...
const diff = Math.round((d - today) / (1000 * 60 * 60 * 24)); const label = diff === 0 ? “Aujourd’hui” : diff === 1 ? “Demain” : diff === -1 ? “Hier” : null; return {
day: d.toLocaleDateString(“fr-FR”, { weekday: “long” }),
date: d.toLocaleDateString(“fr-FR”, { day: “numeric”, month: “long”, year: “numeric” }), label, }; };
...
const applyDrop = () => {
if (!dragItem.current || !dragOver.current) { resetDrag(); return; } const { dateStr, localIdx: from } = dragItem.current; const { dateStr: overDate, localIdx: to } = dragOver.current; if (dateStr !== overDate || from === to) { resetDrag(); return; }

```
const dayStops = [...groupedRef.current[dateStr]]; const [moved] = dayStops.splice(from, 1); dayStops.splice(to, 0, moved);

const originalIndices = groupedRef.current[dateStr].map(s => s.originalIdx); const newStops = [...stopsRef.current]; dayStops.forEach((stop, i) => {
  newStops[originalIndices[i]] = { ville: stop.ville, date: stop.date, note: stop.note, categorie: stop.categorie }; });
...
// Touch starts on the handle: begin drag const handleHandleTouchStart = (e, dateStr, localIdx) => { touchOnHandle.current = true; dragItem.current = { dateStr, localIdx }; setDraggingKey(`${dateStr}-${localIdx}`);
};
...
if (!el) return;
const onTouchMove = (e) => {
if (!touchOnHandle.current || !dragItem.current) return; e.preventDefault(); // block scroll only during a handle-initiated drag const touch = e.touches[0]; const target = document.elementFromPoint(touch.clientX, touch.clientY); if (target) { const stopEl = target.closest(”[data-drag-idx]”);
if (stopEl) {
dragOver.current = {
...
el.addEventListener(“touchmove”, onTouchMove, { passive: false }); return () => el.removeEventListener(“touchmove”, onTouchMove); }, []);

return (
<div className="section">
{!showAdd && editIdx === null && (
<button className=“btn-add-trip” onClick={() => setShowAdd(true)}> <span className="btn-add-icon">+</span> Ajouter une étape </button> )} {showAdd && ( <StopForm title=“Nouvelle étape”
initial={{ ville: “”, date: addDate, note: “”, categorie: “avion” }} onSave={addStop}
onCancel={() => { setShowAdd(false); setAddDate(””); }} /> )} {editIdx !== null && ( <StopForm title=“Modifier l’étape”
...
<p className="empty-title">Aucune étape planifiée</p> <p className="empty-sub">Ajoutez des étapes avec des dates pour voir votre planning</p> </div> )}
            ...
            {dayStops.map((stop, si) => {
              ...
              return (
                <div
                  ...
                  // Desktop: drag only fires if it started on the handle (draggable=false by default)
                >
                  <div className="planning-stop-inner" style={{ borderLeftColor: cat.color }}>
                    <div className="planning-stop-top">
                      {/* Category badge */}
                      <div className="stop-cat-badge" style={{ background: cat.color + "18", color: cat.color }} title={cat.label}>
                        <span>{cat.emoji}</span>
                      </div>
                        ...
                        {canDrag && (
                          <span
                            ...
                            onDragStart={(e) => {
                              ...
                              setDraggingKey(key);
                              // Store on the event so the parent div knows it's a handle drag
...
const [form, setForm] = useState({ label: “”, montant: “”, categorie: “Transport” }); const [showForm, setShowForm] = useState(false); const [showDetail, setShowDetail] = useState(false); const categories = [“Transport”, “Hébergement”, “Resto”, “Activités”, “Autre”]; const catEmoji = { Transport: “✈️”, Hébergement: “🏠”, Resto: “🍽️”, Activités: “🎯”, Autre: “📦” }; const catColors = { Transport: “#FF5A5F”, Hébergement: “#00A699”, Resto: “#FC642D”, Activités: “#484848”, Autre: “#767676” }; const items = budget.items || []; const total = items.reduce((s, i) => s + parseFloat(i.montant || 0), 0); const add = () => { if (!form.label.trim() || !form.montant) return; save(“budget”, { …budget, items: […items, form] }); setForm({ label: “”, montant: “”, categorie: “Transport” }); setShowForm(false); }; const remove = (i) => save(“budget”, { …budget, items: items.filter((_, idx) => idx !== i) }); const byCategorie = categories.reduce((acc, cat) => { acc[cat] = items.filter((i) => i.categorie === cat); return acc; }, {});

return (
<div className="section">
<div className=“budget-hero” onClick={() => setShowDetail(!showDetail)} style={{ cursor: “pointer” }}> <p className="budget-hero-label">Total des dépenses</p> <p className="budget-hero-amount">{total.toFixed(2)} €</p> <div className="budget-chips"> {categories.filter(c => byCategorie[c].length > 0).map(cat => ( <span key={cat} className="budget-chip">{catEmoji[cat]} {byCategorie[cat].reduce((s, i) => s + parseFloat(i.montant), 0).toFixed(0)}€</span> ))} </div> <p style={{ fontSize: “0.75rem”, opacity: 0.7, marginTop: “0.75rem” }}>{showDetail ? “▲ Masquer le détail” : “▼ Voir le détail par catégorie”}</p> </div> {showDetail && ( <div className="detail-card"> {categories.filter(c => byCategorie[c].length > 0).map(cat => { const catTotal = byCategorie[cat].reduce((s, i) => s + parseFloat(i.montant), 0); const pct = total > 0 ? (catTotal / total) * 100 : 0; return ( <div key={cat} className="detail-row"> <div className="detail-top"> <span className="detail-label">{catEmoji[cat]} {cat}</span> <span className=“detail-amount” style={{ color: catColors[cat] }}>{catTotal.toFixed(2)} €</span> </div> <div className="detail-bar"><div className=“detail-bar-fill” style={{ width: `${pct}%`, background: catColors[cat] }} /></div> <span className="detail-pct">{Math.round(pct)}% du total</span> </div> ); })} </div> )} {!showForm && <button className=“btn-add-trip” onClick={() => setShowForm(true)}><span className="btn-add-icon">+</span> Ajouter une dépense</button>} {showForm && ( <div className="form-card"> <div className="form-header"><span>Nouvelle dépense</span><button className=“btn-close” onClick={() => setShowForm(false)}>✕</button></div>
<div className="form-body">
<div className="input-group"><label className="input-label">Description</label><input className=“inp” placeholder=“Ex: Vol Paris-Rome” value={form.label} onChange={(e) => setForm({ …form, label: e.target.value })} /></div> <div className="input-row"> <div className=“input-group” style={{ flex: 1 }}><label className="input-label">Montant (€)</label><input className=“inp” type=“number” placeholder=“0.00” value={form.montant} onChange={(e) => setForm({ …form, montant: e.target.value })} /></div> <div className=“input-group” style={{ flex: 1 }}><label className="input-label">Catégorie</label><select className=“inp” value={form.categorie} onChange={(e) => setForm({ …form, categorie: e.target.value })}>{categories.map((c) => <option key={c}>{c}</option>)}</select></div>
</div>
<button className="btn-primary" onClick={add}>Ajouter</button> </div> </div> )} {items.length === 0 && !showForm && <div className="empty-state"><div className="empty-icon">💳</div><p className="empty-title">Suivez vos dépenses</p><p className="empty-sub">Gardez un œil sur votre budget de voyage</p></div>}
{categories.map((cat) => byCategorie[cat].length > 0 ? ( <div key={cat} className="budget-group"> <div className=“group-header” style={{ borderLeft: `3px solid ${catColors[cat]}` }}><span>{catEmoji[cat]} {cat}</span><span className="group-total">{byCategorie[cat].reduce((s, i) => s + parseFloat(i.montant), 0).toFixed(2)} €</span></div> {byCategorie[cat].map((item, gi) => { const realIdx = items.findIndex((x) => x === item); return ( <div key={gi} className="budget-item"> <span className="item-label">{item.label}</span>
<div style={{ display: “flex”, alignItems: “center”, gap: “0.75rem” }}> <span className="item-amount">{parseFloat(item.montant).toFixed(2)} €</span> <button className=“btn-icon small” onClick={() => remove(realIdx)}>🗑️</button>
</div>
</div>
);
})}
...
const done = todos.filter((t) => t.done).length; const pct = todos.length ? Math.round((done / todos.length) * 100) : 0; const add = () => { if (!input.trim()) return; save(“todos”, […todos, { text: input, done: false }]); setInput(””); }; const toggle = (i) => save(“todos”, todos.map((t, idx) => idx === i ? { …t, done: !t.done } : t)); const remove = (i) => save(“todos”, todos.filter((_, idx) => idx !== i));
...
<div className="progress-bar"><div className=“progress-fill” style={{ width: `${pct}%` }} /></div> </div> )} <div className="todo-input-row"> <input className=“inp todo-inp” placeholder=“Ajouter une tâche…” value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === “Enter” && add()} /> <button className="btn-primary btn-add-inline" onClick={add}>Ajouter</button> </div> {todos.length === 0 && <div className="empty-state"><div className="empty-icon">📋</div><p className="empty-title">Votre liste de tâches</p><p className="empty-sub">Passeport, visas, réservations… tout ici !</p></div>} <div className="todo-list"> {todos.filter(t => !t.done).map((t) => { const ri = todos.findIndex((x) => x === t); return (<div key={ri} className="todo-item"><button className=“todo-check” onClick={() => toggle(ri)} /><span className="todo-text">{t.text}</span><button className=“btn-icon small” onClick={() => remove(ri)}>🗑️</button></div>);
})}
{todos.filter(t => t.done).length > 0 && (<> <p className="done-separator">Terminées</p>
{todos.filter(t => t.done).map((t) => {
const ri = todos.findIndex((x) => x === t); return (<div key={ri} className="todo-item done"><button className=“todo-check checked” onClick={() => toggle(ri)}>✓</button><span className="todo-text">{t.text}</span><button className=“btn-icon small” onClick={() => remove(ri)}>🗑️</button></div>);
})}
</>)}
</div>
</div>
...
// ─── VOYAGE DETAIL ────────────────────────────────────────────────────────────
function VoyageDetail({ voyageId, voyage, saveKey, onBack }) { const [tab, setTab] = useState(0); const meta = voyage.meta || {}; const stops = voyage.stops || []; const budget = voyage.budget || { items: [] }; const todos = voyage.todos || []; const save = (key, val) => saveKey(voyageId, key, val);

return (
<>
<div className="header">
<button className="btn-back" onClick={onBack}>← Tous les voyages</button> <div className="header-eyebrow">{meta.destination || “Destination”}</div>
<h1 className="header-title">{meta.name || “Mon Voyage”}</h1> {meta.dates && <p className="header-dates">📅 {meta.dates}</p>} <div className="tabs"> {TABS.map((t, i) => ( <button key={i} className={`tab ${tab === i ? "active" : ""}`} onClick={() => setTab(i)}>{TAB_ICONS[i]} {t}</button> ))} </div> </div> {tab === 0 && <Planning stops={stops} save={save} />} {tab === 1 && <Budget budget={budget} save={save} />} {tab === 2 && <TodoList todos={todos} save={save} />} </> ); }

// ─── VOYAGE LIST ──────────────────────────────────────────────────────────────
function VoyageList({ voyages, onCreate, onSelect, onDelete }) { const [showForm, setShowForm] = useState(false); const [form, setForm] = useState({ name: “”, destination: “”, dates: “” }); const [confirmDelete, setConfirmDelete] = useState(null);

const submit = () => {
if (!form.name.trim()) return;
const id = onCreate(form.name, form.destination, form.dates); setForm({ name: “”, destination: “”, dates: “” }); setShowForm(false); onSelect(id); };
...
<h1 className="home-title">Mes Voyages ✈️</h1> </div> <div className="section"> {!showForm && <button className=“btn-new-voyage” onClick={() => setShowForm(true)}><span className="btn-new-icon">+</span> Nouveau voyage</button>} {showForm && ( <div className="form-card"> <div className="form-header"><span>Nouveau voyage</span><button className=“btn-close” onClick={() => setShowForm(false)}>✕</button></div>
<div className="form-body">
<div className="input-group"><label className="input-label">Nom du voyage</label><input className=“inp” placeholder=“Ex: Road trip Espagne” value={form.name} onChange={(e) => setForm({ …form, name: e.target.value })} /></div> <div className="input-group"><label className="input-label">Destination</label><input className=“inp” placeholder=“Ex: Barcelone, Madrid…” value={form.destination} onChange={(e) => setForm({ …form, destination: e.target.value })} /></div> <div className="input-group"><label className="input-label">Dates</label><input className=“inp” placeholder=“Ex: 15 juin - 30 juin 2025” value={form.dates} onChange={(e) => setForm({ …form, dates: e.target.value })} /></div> <button className="btn-primary" onClick={submit}>Créer le voyage</button> </div> </div> )} {list.length === 0 && !showForm && ( <div className=“empty-state” style={{ paddingTop: “4rem” }}> <div className="empty-icon">🌍</div>
<p className="empty-title">Aucun voyage pour l’instant</p> <p className="empty-sub">Créez votre premier voyage pour commencer à planifier !</p> </div> )} <div className="voyage-list"> {list.map(([id, voyage]) => { const meta = voyage.meta || {}; const stops = voyage.stops || []; const todos = voyage.todos || []; const budget = voyage.budget || { items: [] }; const total = (budget.items || []).reduce((s, i) => s + parseFloat(i.montant || 0), 0); const doneTodos = todos.filter(t => t.done).length; return ( <div key={id} className=“voyage-card” onClick={() => onSelect(id)}> <div className="voyage-card-top"> <div> <p className="voyage-destination">{meta.destination || “Destination”}</p>
<h2 className="voyage-name">{meta.name}</h2>
{meta.dates && <p className="voyage-dates">📅 {meta.dates}</p>} </div> <button className=“btn-delete-voyage” onClick={(e) => { e.stopPropagation(); setConfirmDelete(id); }}>🗑️</button>
</div>
<div className="voyage-stats">
<span className="voyage-stat">🗺️ {stops.length} étape{stops.length > 1 ? “s” : “”}</span> <span className="voyage-stat">💰 {total.toFixed(0)} €</span> <span className="voyage-stat">✅ {doneTodos}/{todos.length} tâches</span> </div> </div> ); })} </div> </div> {confirmDelete && ( <div className=“modal-overlay” onClick={() => setConfirmDelete(null)}> <div className=“modal” onClick={(e) => e.stopPropagation()}> <p className="modal-title">Supprimer ce voyage ?</p> <p className="modal-sub">Cette action est irréversible.</p> <div className="modal-actions"> <button className=“btn-cancel” onClick={() => setConfirmDelete(null)}>Annuler</button>
<button className=“btn-danger” onClick={() => { onDelete(confirmDelete); setConfirmDelete(null); }}>Supprimer</button> </div> </div> </div> )} </> ); }

// ─── APP ──────────────────────────────────────────────────────────────────────
export default function App() {
const { voyages, loaded, createVoyage, deleteVoyage, saveVoyageKey } = useVoyages(); const [selectedId, setSelectedId] = useState(null);
...
<>
<style>{`
@import url(‘https://urldefense.com/v3/__https://fonts.googleapis.com/css2?family=Plus*Jakarta*Sans:wght@400;500;600;700&display=swap__;Kys!!IY5JXqZAIQ!6J6VWHMxlNKAgX2NW-D5q3GT4aPjR9gxi0DzPgELGvFczTe7YY9_8n8Sj1XIpcystU57CKDJd4RDKFuafcFB$ ’); *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; } body { background: #F7F7F7; color: #222; font-family: ‘Plus Jakarta Sans’, sans-serif; min-height: 100vh; } .app { max-width: 480px; margin: 0 auto; min-height: 100vh; background: #fff; box-shadow: 0 0 40px rgba(0,0,0,0.08); padding-bottom: 100px; }
...
```
    ...
    .drag-handle { font-size: 1.3rem; color: #CCCCCC; user-select: none; cursor: grab; padding: 0.2rem 0.3rem; border-radius: 6px; line-height: 1; touch-action: none; }
